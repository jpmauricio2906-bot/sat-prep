...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function Dashboard({progress,onStartTopic,onPracticeTest,onMockTest, bankIssues=[], validatorReport=null, auditEnabled=false, onExportMatrix, onExportIssues){
  const T=useTheme();
          ...
          <div style={{display:"flex",gap:10}}>
            {auditEnabled && (
              <>
                ...
                <a href="/question_matrix.csv" download style={{padding:"8px 10px",borderRadius:10,border:"1px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.04)",color:"white",textDecoration:"none",display:"inline-flex",alignItems:"center"}}>
                  ⬇️ Validator Matrix
                </a>
              </>
            )}
            ...
            <a href="/question_issues.json" download style={{padding:"8px 10px",borderRadius:10,border:"1px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.04)",color:"white",textDecoration:"none",display:"inline-flex",alignItems:"center"}}>
              ⬇️ Validator Issues
            </a>
            {auditEnabled && validatorReport && (
              <span style={{opacity:0.9,fontSize:12}}>
                Validator issues: <b>{validatorReport.issueCount}</b>
              </span>
            )}
          </div>
        </div>
...
export default function App(){
  ...
  // Load build-time validator report (generated by scripts/validateQuestions.mjs)
  useEffect(() => {
    if (!auditEnabled) return;
    fetch('/question_issues.json')
      .then(r => (r && r.ok) ? r.json() : null)
      .then(data => setValidatorReport(data))
      .catch(() => setValidatorReport(null));
  }, [auditEnabled]);
;
